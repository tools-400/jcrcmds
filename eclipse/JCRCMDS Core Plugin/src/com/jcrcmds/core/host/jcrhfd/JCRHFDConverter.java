...
public class JCRHFDConverter {
	...
	public void convertAsync(final String[] sourceLines,
			final String memberType, final IResultReceiver receiver)
			throws Exception {
		...
		service.busyCursorWhile(new IRunnableWithProgress() {
			public void run(IProgressMonitor arg0) {
				try {
					...
					MessageDialogAsync.displayError(ExceptionHelper
	...
	private String[] convert(String[] sourceLines, String memberType)
			throws Exception {
		...
		try {
			...
			MessageDialogAsync
					.displayError(Messages.Could_not_retrieve_current_library);
			return null;
		}
		...
		try {
			...
			MessageDialogAsync
					.displayError(Messages.Could_not_set_current_library_to_A);
			return null;
		}
		...
		try {
			...
			RemoteLibrary library = new RemoteLibrary(connectionName,
					workLibrary);

			if (!library.exists()) {
				MessageDialogAsync.displayError(Messages.bind(
						Messages.Library_A_not_found, workLibrary));
				return null;
			}

			RemoteFile sourceFile = new RemoteFile(connectionName, workLibrary,
					workFile);
				...
				if (message != null) {
					MessageDialogAsync
							.displayError(Messages
									.bind(Messages.Could_not_create_source_file_B_in_library_A_Reason_C,
											new String[] { workLibrary,
													workFile, message }));
					return null;
				}
			}

			RemoteSourceMember sourceMember1 = new RemoteSourceMember(
					connectionName, workLibrary, workFile, workMember1);
			...
			if (!sourceMember1.exists()) {
				...
				if (message != null) {
					MessageDialogAsync
							.displayError(Messages
									.bind(Messages.Could_not_create_source_member_C_in_file_B_in_library_A_Reason_D,
											new String[] { workLibrary,
													workFile, workMember1,
													message }));
					return null;
				}
			} else {
				...
				if (message != null) {
					MessageDialogAsync
							.displayError(Messages
									.bind(Messages.Could_not_clear_source_member_C_in_file_B_in_library_A_Reason_D,
											new String[] { workLibrary,
													workFile, workMember1,
													message }));
					return null;
			...
			sourceMember1
					.setTypeAndDescription(memberType, "JCRHFD Input Data"); //$NON-NLS-1$

			RemoteSourceMember sourceMember2 = new RemoteSourceMember(
					connectionName, workLibrary, workFile, workMember2);
			...
			String message = sourceMember2.setTypeAndDescription(memberType,
					"JCRHFD Output Data"); //$NON-NLS-1$
			if (message != null) {
			...
			String jcrHfdCommand = preferences.getProductLibrary()
					+ "/" + preferences.getJcrHfdCommand(); //$NON-NLS-1$
			...
			try {
				...
				Logger.logError(
						"*** Could not restore current library to: " + currentLibrary + " ***", e); //$NON-NLS-1$ //$NON-NLS-2$
				MessageDialogAsync
						.displayError(Messages.Could_restore_current_library_to_A);
			}
		}
