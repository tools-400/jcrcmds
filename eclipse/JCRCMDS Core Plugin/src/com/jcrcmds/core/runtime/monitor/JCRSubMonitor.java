 ...
 *******************************************************************************/
package com.jcrcmds.core.runtime.monitor;
 ...
 *         // use 0.01% of the space remaining in the monitor to process the next
 *         // node.
 ...
 */
public final class JCRSubMonitor implements IProgressMonitorWithBlocking {
    ...
    private static final class RootInfo {
        ...
        public void clearBlocked() {
            if (root instanceof IProgressMonitorWithBlocking) ((IProgressMonitorWithBlocking)root).clearBlocked();
        }

        public void setBlocked(IStatus reason) {
            if (root instanceof IProgressMonitorWithBlocking) ((IProgressMonitorWithBlocking)root).setBlocked(reason);
        }
     ...
     */
    private JCRSubMonitor(RootInfo rootInfo, int totalWork, int availableToChildren, int flags) {
        root = rootInfo;
     ...
     * </p>
     * 
     * @param monitor monitor to convert to a SubMonitor instance or null.
     *        Treats null as a new instance of <code>NullProgressMonitor</code>.
     * @return a SubMonitor instance that adapts the argument
     */
    public static JCRSubMonitor convert(IProgressMonitor monitor) {
     ...
     * @return a SubMonitor instance that adapts the argument
     */
    public static JCRSubMonitor convert(IProgressMonitor monitor, int work) {
     ...
     *        progress anywhere.
     * @param taskName user readable name to pass to monitor.beginTask. Never
     *        null.
     ...
     */
    public static JCRSubMonitor convert(IProgressMonitor monitor, String taskName, int work) {
        if (monitor == null) monitor = new NullProgressMonitor();

        // Optimization: if the given monitor already a SubMonitor, no
        // conversion is necessary
        if (monitor instanceof JCRSubMonitor) {
            monitor.beginTask(taskName, work);
            return (JCRSubMonitor)monitor;
        }

        monitor.beginTask(taskName, MINIMUM_RESOLUTION);
        return new JCRSubMonitor(new RootInfo(monitor), MINIMUM_RESOLUTION, work, SUPPRESS_NONE);
    }
     ...
     * @return the receiver
     */
    public JCRSubMonitor setWorkRemaining(int workRemaining) {
    ...
    private int consume(double ticks) {
        if (totalParent == 0 || totalForChildren == 0) // this monitor has no
                                                       // available work to
                                                       // report
            return 0;
        ...
        else if (usedForChildren < 0.0) usedForChildren = 0.0;
    ...
    public void setTaskName(String name) {
        if ((flags & SUPPRESS_SETTASKNAME) == 0) root.setTaskName(name);
    }
    ...
    public void beginTask(String name, int totalWork) {
        if ((flags & SUPPRESS_BEGINTASK) == 0 && name != null) root.setTaskName(name);
     ...
     */
    public void subTask(String name) {
        if ((flags & SUPPRESS_SUBTASK) == 0) root.subTask(name);
    }
     ...
     *         SubMonitor
     */
    public JCRSubMonitor newChild(int totalWork) {
     ...
     *         SubMonitor
     */
    public JCRSubMonitor newChild(int totalWork, int suppressFlags) {
        ...
        // though the child is
        // delegating to its parent, even though it is actually talking directly
        // to the root.
        // This means that we need to compute the flags such that - even if a
        // label isn't
        // suppressed by the child - if that same label would have been
        // suppressed when the
        // child delegated to its parent, the child must explicitly suppress the
        // label.
        ...
        if ((flags & SUPPRESS_SETTASKNAME) != 0) {
            ...
            // child will ignore
            // labels passed to either beginTask or setTaskName - since both
            // delegate to setTaskName
            // on the parent
        ...
        // there
        // is no method on the child that would delegate to beginTask on the
        // parent.
        childFlags |= suppressFlags;

        JCRSubMonitor result = new JCRSubMonitor(root, consume(totalWorkDouble), (int)totalWorkDouble, childFlags);
